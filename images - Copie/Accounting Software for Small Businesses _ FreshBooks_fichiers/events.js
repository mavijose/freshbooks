...
function currentLandingURL(){
  ...
  return [landing_url, current_window_url_param];
}
...
function loadConv(args){
    ...
    img.onerror = remove;
    img.onload = remove;
    img.src = sa_url + "/conv?cid=" + args + "&url=" + landing_url + "&rnd=" + Math.random();
...
function loadRtHelper(url, args) {
    ...
    img.onerror = remove;
    img.onload = remove;
    img.src = url + "/rt?sid=" + args + "&url=" + landing_url;
    document.body.appendChild(img);
}
...
function loadTs(args){
  ...
  res = currentLandingURL();
  landing_url = res[0];
  current_window_url_param = res[1];
  uid = uid.replace(/#/g, "");
...
// Checking if the click was redirected from the a different domain than the domain the user is on
  if ((referrer == "none" || !document.referrer.includes(window.location.hostname)) && current_window_url_param.indexOf("utm_source") >= 0){
    ...
    if (is_dev) {
      makeCorsRequest('http://192.168.50.4' + '/tec_ts?' + current_window_url_param + "&uid="+ uid + "&landing_url=" + landing_url + "&host=" + globals.host + "&sec=0" + "&referrer=" + referrer + "&url=" +landing_url);
    } else {
      makeCorsRequest('https://srv.stackadapt.com' + '/tec_ts?' + current_window_url_param + "&uid="+ uid + "&landing_url=" + landing_url + "&host=" + globals.host + "&sec=0" + "&referrer=" + referrer + "&url=" + landing_url);
    }
  }
  ////////////////////////////////////////////////
...
function loadSyncPixelHelper() {
    var userIdV2 = "0-2cd079ad-73d7-4719-6e1a-29192b237e6b$ip$41.66.28.80";
    var userIdB64 = "LNB5rXPXRxluGikZKyN-aylCHFA";

    sync_pixel_map.forEach(function (pixel) {

        var userId = "";
        if (pixel["encoding"] == "plain") {
            userId = userIdV2;
        } else if (pixel["encoding"] == "b64") {
            userId = userIdB64;
        }
        if (userId.length > 0) {
            try {
...
try {
  ...
  var sync_pixel_map=[{"url":"https://cm.g.doubleclick.net/pixel?google_nid=stackadapt_usd&google_ula=460104972&google_hm=","encoding":"b64"},
      {"url":"https://x.bidswitch.net/sync?dsp_id=188&user_group=1&user_id=","encoding":"b64"},
      {"url":"https://ib.adnxs.com/setuid?entity=328&code=","encoding":"plain"}];
